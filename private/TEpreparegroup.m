...
% parameters.
% 2013-06-26: PW commented out all checks for equality except for toi and
% embedding dimension (dim)
% 2013-06-26: PW added field .nrchannels to output structure (field is
% required by TEgroup_stats.m)
% 2013-06-26: PW commented out the setting of a fixed tau for the whole
% group (tau is now individually defined within TEgroup_calculate for each
% subject)
...
% vectors
Snrchannelcombi = nan(1,NrSubjects);
...
for ii = 1:NrSubjects
    ...
    Snrchannelcombi(ii) = size(Data{ii}.TEprepare.channelcombi,1);
    % toi
    Stoifrom(ii) = Data{ii}.TEprepare.cfg.toi(1);
    Stoito(ii) = Data{ii}.TEprepare.cfg.toi(2);
    % tau
    if strcmp(Data{ii}.TEprepare.cfg.optimizemethod, 'cao');
...
%     Snrtrials(ii,:) = Data{ii}.TEprepare.nrtrials;
    ...
    %Scalctype{ii} = Data{ii}.TEprepare.cfg.TEcalctype; % PW redundant: is
    %read out below
...
if strcmp(Soptmethod{1},'cao')
    ...
    for ii = 2:NrSubjects
        if ~isequal(Scaodim{1},Scaodim{ii})
            fprintf('\n');
...
%         disp(Resvector)
%         fprintf('\n');
%         error('TRENTOOL ERROR: repPred were not the same for all subjects, see help!')
%     end
    for ii = 2:NrSubjects
        if ~isequal(Sragdim{1},Sragdim{ii})
            fprintf('\n');
...
%% check prediction time u for equality
% -------------------------------------------------------------------------
for jj = 2:NrSubjects
    if ~isequal(Su{1},Su{jj})
        fprintf('\n');
...
elseif min(Snrchannelcombi) ~= max(Snrchannelcombi)
    Resvector(:,2)=Snrchannelcombi;
    disp(Resvector)
...
for ii = 1:NrSubjects
    for jj = ii:NrSubjects
        if ii~=jj
            ...
            % channel
            for c1 = 1:Snrchannelcombi(ii)
                for c2 = 1:Snrchannelcombi(jj)
                    if strcmp(Schannel{ii}(c1,1),Schannel{jj}(c2,1)) && strcmp(Schannel{ii}(c1,2),Schannel{jj}(c2,2)) 
                        comp1 = comp1 + 1;
                    end
...
% these settings are later used within TEgroup_calculate as parameters for
% TE analysis (are passed to transferentropy.m as cfg)
...
% maxtrials
TEgroupprepare.nrchannelcombis = size(Schannel{1},1);
%TEgroupprepare.nrchannels = size(Data{ii}.TEprepare.channellabel,1);
disp('###################################################################################################################################  !!!!!!')
warning('Commented out channellabel -> this is needed later!')
